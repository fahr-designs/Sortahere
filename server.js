// ─── 1. LOAD ENV VARS ────────────────────────────────────────────────────────
// Must be first. Reads .env and injects into process.env before anything else runs.
...
// ─── 3. CONFIG ───────────────────────────────────────────────────────────────
// Read from process.env — populated by dotenv locally, by hPanel in production.
// The || fallback is only a safety net. Always set these explicitly.
...
// Better to crash at startup than to fail silently mid-request.
if (!ADMIN_TOKEN) {
...
// ─── 4. RESEND CLIENT ────────────────────────────────────────────────────────
// Instantiated once at startup — not inside a route handler.
// The Resend SDK handles the HTTP communication to resend.com's API.
const resend = new Resend(RESEND_API_KEY);

// ─── 5. DATABASE SETUP ───────────────────────────────────────────────────────
// Opens (or creates) the SQLite file at DB_PATH.
// better-sqlite3 is synchronous — no promises, no callbacks.
// This is appropriate at this scale and simplifies the code significantly.
const db = new Database(DB_PATH);

// CREATE TABLE IF NOT EXISTS means this is safe to run every time the server starts.
// If the table already exists, it's a no-op.
db.exec(`
...
// ─── 7. RATE LIMITER ─────────────────────────────────────────────────────────
// Applied only to POST /rsvp — not the whole app.
...
// POST /rsvp
// Public route — any guest can hit this.
...
app.post('/rsvp', rsvpLimiter, (req, res) => {
  ...
  if (attending !== 0 && attending !== 1)
    return res.status(400).json({ error: 'Please select whether you are attending.' });
...
// POST /admin/send-invites
// Protected route — only you can trigger this.
// Reads guests.json, sends each address an invitation email via Resend.
app.post('/admin/send-invites', async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(guests)) throw new Error('guests.json must be a JSON array');
  } catch (err) {
  ...
  //    Resend's free tier handles this volume easily.
